...
public class BoardController {
	...
	public String list(@RequestParam(name = "page", defaultValue = "1") int current_page,
		...
		try {
			...
			if (! keyword.isBlank()) {  // if(kwd.length() != 0) {
	...
	public String article(@RequestParam(name = "num") long number,
		...
		try {
			...
			boardService.updateHitCount(number);

			Board dto = Objects.requireNonNull(boardService.findById(number));
	...
	@GetMapping("update")
	public String updateForm(@RequestParam(name = "num") long number,
			@RequestParam(name = "page") String pageNo,
			Model model) throws Exception {
		// 수정 폼
		try {
	...
	public String updateSubmit(Board dto,
			@RequestParam(name = "page") String pageNo) throws Exception {
		// 수정 완료
		try {
	...
	public String delete(@RequestParam(name = "num") long number,
			@RequestParam(name = "page") String pageNo,
			@RequestParam(name = "schType", defaultValue = "all") String searchType,
			@RequestParam(name = "kwd", defaultValue = "") String keyword) throws Exception {
		// 게시글 삭제
		String query = "page=" + pageNo;
		try {
			keyword = URLDecoder.decode(keyword, "utf-8");
